...
from .patchnce_time import PatchNCELossTime
import pdb
...
class SBModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--temp_nce_T', type=float, default=0.1, help='temperature for NCE loss')
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            for nce_layer in self.nce_layers:
                self.criterionNCE.append(PatchNCELossTime(opt).to(self.device))
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        # all_list = random.sample(all_list, num_to_select)
        
        all_list = torch.tensor(all_list)
        
        for feat_idx in range(len(feat_q)):
            batchsize, num_channels, height, width = feat_q[feat_idx].size()
            channels_per_group = num_channels // self.opt.temp_nc
            feat_q_tgt = feat_q[feat_idx].view(batchsize, self.opt.temp_nc, channels_per_group, height, width)
            
            
            shuffle_idx = all_list.cuda()
            feat_q_shu = feat_q_tgt[:, shuffle_idx]
            feat_q_shu = feat_q_shu.view(batchsize * len(shuffle_idx), -1, height, width)
            feat_q_shuffle.append(feat_q_shu)
        ...
        feat_q_pool_shuffle, _ = self.netF(feat_q_shuffle, self.opt.num_patches, sample_ids)
        

        total_nce_loss = 0.0
        for f_q, f_k, f_q_shuffle, crit, nce_layer in zip(feat_q_pool, feat_k_pool, feat_q_pool_shuffle, self.criterionNCE, self.nce_layers):
            loss = crit(f_q, f_k, f_q_shuffle) * self.opt.lambda_NCE
