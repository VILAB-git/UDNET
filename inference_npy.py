...
def event_npy(event, name, opt):

    
    save_path2 = f'./results/{opt.name}_epoch_{opt.epoch}'

    parent_path2 = Path(save_path2)
    event = torch.clamp(event, -1.0, 1.0)
    event = event.squeeze(0).detach().cpu().numpy()

    os.makedirs(parent_path2, exist_ok = True)
    file_name = name + '.npy'

    np.save(parent_path2 / str(file_name), event)
...
if __name__ == '__main__':
    ...
    for i, (data,data2) in enumerate(zip(dataset,dataset2)):
        ...
        event_npy(visuals['fake_5'], data['A_paths'][0].split('/')[-1].split('.')[0], opt)
